<?php
    ...
    class DateTimeUtil {
        ...
        public static function format_date_interval(DateInterval $date_interval, $precision = 2, $long_labels = false, $signed = true) {
            ...
            for ($i = 0; $i < $precision; $i++) {
                if ($interval_parts) {
                    ...
                    $displayable_label = ($long_labels ? " " : "").substr(array_shift($interval_part_keys), 1).($long_labels && $displayable_part != 1 ? "s" : "")." ";
        ...
        public static function get_business_days(DateTime $older_start_date, DateTime $newer_end_date = null) {
            ...
            if(strtotime($older_start_date->format(DateTimeUtil::DATE_FORMAT_DISPLAY_LONG)) < strtotime($newer_end_date->format(DateTimeUtil::DATE_FORMAT_DISPLAY_LONG))) {
                $count = 0;
                while(strtotime($older_start_date->format(DateTimeUtil::DATE_FORMAT_DISPLAY_LONG)) < strtotime($newer_end_date->format(DateTimeUtil::DATE_FORMAT_DISPLAY_LONG))) {
                    $older_start_date = DateTimeUtil::add($older_start_date, new DateInterval("P1D"));
                    if($older_start_date->format("N") < 6) { // We only count if the older date hasn't hit a weekend date 6-7
                        $count++;
                    }
                }
                return $count;
