...
//        $this->variables[$name] = $value;
    ...
    protected function _get_output($renderHeader) {
        ...
        if($renderHeader) {
            if (file_exists(APP_PATH . 'views/' . $this->_controller . '/header.php')) {
                 $output['header'] = array('render' => true, 'path' => APP_PATH . 'views/' . $this->_controller . '/header.php');
            } else {
                $output['header'] = array('render' => true, 'path' => APP_PATH . 'views/header.php');
            }
            if (file_exists(APP_PATH . 'views/'. $this->_controller . '/footer.php')) {
               $output['footer'] = array('render' => true, 'path' => APP_PATH . 'views/'. $this->_controller . '/footer.php');
            } else {
        ...
        if($this->override_page) {
            if (file_exists(APP_PATH . 'views/' . $this->_controller .'/' . $this->override_page)) {
                $output['content'] = array('render' => true, 'path' => APP_PATH . 'views/' . $this->_controller . '/' . $this->override_page);
            } else if(file_exists(APP_PATH . 'views/' . $this->_controller . '/' . $this->_action . '.php')) {
                $output['content'] = array('render' => true, 'path' => APP_PATH . 'views/' . $this->_controller . '/' . $this->_action . '.php');
            } else {
            ...
            if (file_exists(APP_PATH . 'views/' . $this->_controller . '/' . $this->_action . '.php')) {
                ...
                $output['content'] = array('render' => true, 'path' => APP_PATH . 'views/'.$this->uri->rsegments[0].'/'.$this->_action.'.php');
            } else {
                $output['content'] = array('render' => true, 'path' => 'error');
            }
        }
    ...
    /**
     * @param bool $renderHeader
     *
     * @param bool $_return
     *
     * @return string
     * @internal param int $doNotRenderHeader in the old code there is not arg* in the old code there is not arg
     */
                ...
                if(isset($output[$piece]) && $output[$piece]['render']) {
                    if(file_exists($output[$piece]['path'])) {
                        if((bool) @ini_get('short_open_tag') === false && Config::getItem('rewrite_short_tags') == true) {
                            echo eval('?>'.preg_replace("/;*\s*\?>/", "; ?>", str_replace('<?=', '<?php echo ', file_get_contents($output[$piece]['path']))));
                        } else {
