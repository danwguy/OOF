<?php
    ...
    class Loader {
        ...
        public function view($view, $vars = array(), $return = false) {
            return $this->_load(
                        array('_view' => $view, '_vars' => $this->_object_to_array($vars), '_return' => $return));
        }
        ...
        public static function load($class, $folder = 'libraries', $prefix = '', $args = array()) {
            ...
            if(file_exists(APP_PATH . $folder . '/' . $name . '.php')) {
                $name = Config::getItem('subclass_prefix') . $class;
                if(class_exists($name) === false) {
                    require(APP_PATH . $folder . '/' . $name . '.php');
                }
            }
            if(!$name) {
            ...
            if(!class_exists($class)) {
                if(file_exists(SYS_PATH.$folder.'/'.$class.'.php')) {
                    require_once(SYS_PATH.$folder.'/'.$class.'.php');
                }
            }
        ...
        protected function _load($_data) {
            ...
            if((bool)@ini_get('short_open_tag') === false && Config::getItem('rewrite_short_tags') == true) {
                echo eval('?>' . preg_replace(
                        "/;*\s*\?>/",
                        "; ?>",
                        str_replace('<?=', '<?php echo ', file_get_contents($_path))));
            } else {
                include($_path);
            }
            if($_return) {
                ...
                if(is_array($config_component->_config_paths)) {
                    foreach($config_component->_config_paths as $path) {
                        if(defined('ENVIRONMENT')
                           && file_exists(
                                $path . 'config/' . ENVIRONMENT . '/' . strtolower($class) . '.php')
                        ) {
                            include($path . 'config/' . ENVIRONMENT . '/' . strtolower($class) . '.php');
                            break;
                        } else if(defined('ENVIRONMENT')
                                  && file_exists(
                                $path . 'config/' . ENVIRONEMTN . '/' . ucfirst(strtolower($class)) . '.php')
                        ) {
                    ...
                    if(in_array($subclass, $this->_loaded_files)) {
                        if(!is_null($obj_name)) {
                            ...
                            if(!isset($OOF->$obj_name)) {
                                return $this->_init_class(
                                            $class,
                                            $OOF->config->item('subclass_prefix'),
                                            $params,
                                            $obj_name);
                            }
                        }
