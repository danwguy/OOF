    ...
    class Config {
        ...
        public static function get_config($replacement = array()) {
            ...
            if(!defined("ENVIRONMENT")
               || !file_exists(
                    $path = APP_PATH . 'config/' . ENVIRONMENT . '/site_config.php')
            ) {
        ...
        public function load($file = '', $merge = false, $graceful = false) {
            ...
            foreach($this->_config_paths as $path) {
                ...
                if(!isset($config) || !is_array($config)) {
                    ...
                    OOF::show_error(
                       'Your ' . $file_path . ' file either doesn\'t exist or is not a valid config array');
                }
                if($merge) {
        ...
        public function site_url($uri = null) {
            ...
            if(!$this->item('enable_query_string')) {
                ...
                return $this->item_slashed('base_url') . $this->item_slashed('index_page') . $this->_uri_string($uri)
                       . $suffix;
            } else {
