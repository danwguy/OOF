        ...
        */
        ...
        public function __construct() {
            ...
            if(func_num_args() == 1) {
                ...
                throw new Exception ("Unable to create $class.  The constructor for a $class requires either an integer primary key, array of properties and values, or another $class as a singular argument."
                                     . (isset($arg) ? "The argument given was " . var_export($arg, true)
        ...
        //should only be called from the constructor (or via retrieve()), not by outside classes.
        protected function set_class_variables($data) {
            if($data) {
        ...
        public function create() {
            ...
            try {
                ...
                foreach($columns as $column) {
                    switch($column) {
                        case $primary_key:
                            ...
                            break;
                        case "created_on":
                            $set_values[] = "created_on = " . ($this->created_on ?
                                    "'" . DateTimeUtil::format($this->created_on, DateTimeUtil::DATETIME_FORMAT_MYSQL)
                                    . "'" : "NOW()");
                            break;
                        case "modified_on":
                ...
                $this->$primary_key  = $this->$primary_key
                    ? : $this->creation_id; //if there isn't a primary key set make it the creation id
                $this->after_creation();
                DB::get()->commit_trans();
                ...
                Debug::setContent(CustomException::to_string($e->getMessage()));
//                throw $e;
            }
        }
        ...
        public static function retrieve_multiple(array $ids_or_conditions) {
            $associative_input = array_reduce(
                array_keys($ids_or_conditions),
                function ($assoc_found, $key) {
            ...
            if($associative_input) {
                ...
                $sql .= static::get_primary_key() . " IN (" . implode(
                        ", ",
                        array_fill(0, sizeof($ids_or_conditions), "?")) . ")";
            }
        ...
        public function save($editable_columns_only = true) {
            ...
            try {
                ...
                if(!empty($columns)) {
                    ...
                    throw new Exception("The class " . LanguageUtil::get_class_from_namespace($this)
                                        . " does not have any editable fields.  As such, it can not be saved.");
                }
            } catch(Exception $e) {
                DB::get()->rollback_trans();
                throw $e;
            ...
            if(is_array($field)) {
                foreach($field as $key => $value) {
                    $this->_extraConditions .=
                        '`' . $this->_model . '`.`' . $key . '` = "' . mysql_real_escape_string($value) . '" AND ';
                }
            } else {
                $this->_extraConditions .=
                ...
                foreach($tempResults[$class] as $class_obj) {
                    ...
                    if($this->_hM == 1 && isset($this->hasMany)) {
                        foreach($this->hasMany as $aliasChild => $modelChild) {
                            ...
                            $queryChild   = 'SELECT ' . implode(", ", $childColumns) . ' FROM ' . $fromChild
                                            . ' WHERE ' . $conditionsChild;
                            #echo '<!--'.$queryChild.'-->';
        ...
        */
        ...
        public static function retrieve_object($where_clause, $params = null) {
            if($where_clause) {
                $where_clause =
                    preg_match("/(?i)^\\s*WHERE\\s/", $where_clause) != 0 ? " $where_clause" : " WHERE $where_clause";
            }
        ...
        public static function get_related($name, $arguments) {
            ...
            if(preg_match("/^get_related_(.*?)(ies|s)?_?(ids?)?$/", $name, $matches) && !empty($arguments)) {
                ...
                if(property_exists(
                    $target_class,
                    $fk)
                ) { //check to see that the $target_class has properly named foreign key
