<?php
    ...
    class DB {
        ...
        function connect() {
            ...
            if(!$this->conn) {
                $this->error(
                     "Connection failed at " . DateTimeUtil::format(
                                                           date_create(),
                                                           DateTimeUtil::DATETIME_FORMAT_LOGGING) . " using DB_HOST: "
                     . DB_HOST . ", DB_USER: " . DB_USER);
            }
            mysql_select_db(
                ...
                while($row = mysql_fetch_row($result)) {
                    ...
                    for($i = 0; $i < $num_fields; $i++) {
                        ...
                        if(!isset($assoc_row[$key])) { //if there is a key conflict, give priority to the first
                            if(is_null($value)) { //null values should stay as null objects
                            ...
                            ) { //if the field is a regular date/datetime
                                ...
                                $assoc_row[$key] = $datetime->setTimezone(
                                                            new DateTimeZone(date_default_timezone_get())); //UTC may not be so useful, so convert to the timezone that php is using at the time
                                ...
                                } else if((string)($float_value = (float)$value) === (string)$value
                                ) { //make it a float if there isn't any precision loss
                                ...
                                $field->type,
                                array("blob", "text", "tinytext", "mediumtext", "longtext"))
                            ) { //text fields might be storing serialized arrays
        ...
        function prepare_param($param) {
            ...
            if(is_null($param)) {
                ...
                $param = str_replace(
                    "?",
                    "'||0x3F||'",
                    $param); //specially encode question marks in a param so PHP doesn't think they are another param.
        ...
        function execute($sql, $params = null) {
            ...
            if(defined("LOG_SQL_QUERIES") && LOG_SQL_QUERIES) {
                mysql_query(
                    "INSERT INTO log_sql_queries SET
