    ...
    class ARDO {
        ...
        public function retrieve($id = null) {
            ...
            $sql .= implode(
                ', ',
                array_map(
                    function ($column) use ($base_table) {
            ...
            foreach($columns as $class => $set) {
                $sql .= ', ' . implode(
                        ', ',
                        array_map(
                            function ($column) use ($tables, $class) {
            ...
            for($i = 0, $len = count($this->relates); $i < $len; $i++) {
                ...
                if($first) {
                    $sql .= "\r\n" . " ON " . $base_table . "." . static::get_primary_key() . " = " . $new_table . "."
                            . $base_table . "_id";
                    $first = false;
                } else {

                    $sql .=
                        "\r\n" . " ON " . $old_table . "." . $old_class::get_primary_key() . " = " . $new_table . "."
                        . $old_table . "_id";
                }
            }
